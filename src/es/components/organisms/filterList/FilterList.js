      ...
      this.html = /* html */`
        <ul class="list-items ${data.filterType}">
          ${Array.from(new Set(
            ...
            data.items
            // @ts-ignore
          )).reduce((accumulator, name) => /* html */`${accumulator}<li><a-button ${data.filterActive.some(item => item === name) ? 'class="active"' : ''}  filter-type="${data.filterType}" namespace="button-steps-filter-" tag="${name}" answer-event-name="list-events" active-detail-property-name="fetch:filter:${data.filterType}" request-event-name="request-list-events">${name} </a-button></li>`, '')}
        <ul>
      `
    }
